...
4. **执行优化**（强制规则：每个章节的内容修改必须由独立 subagent 完成，禁止 main agent 直接修改任何 `.md` 内容文件）：
   - 为每个需要修改的章节单独派发一个 subagent，明确告知修改内容
   - 多个章节可并行派发，subagent 各自独立执行并验证构建
   - 通用建议（多个章节适用）：列出清单，询问用户是否批量应用，再批量派发 subagent
5. **输出审查报告**：汇总所有 subagent 的改动结果，列出每处改动及改动理由
